...
import type { ActiveListingSummary } from "./client"
...
function getImages(item: Record<string, unknown>): string[] {
  ...
  if (Array.isArray(pictureUrls)) {
    return pictureUrls.map(stringValue).filter(Boolean)
  }
...
function getPrice(item: Record<string, unknown>, summary: ActiveListingSummary): number {
  ...
  if (typeof startPrice === "number") return startPrice
  if (isRecord(startPrice)) return Number(startPrice["#text"] ?? summary.currentPrice)

  return summary.currentPrice
}

function getDateAdded(item: Record<string, unknown>): string {
  const listingDetails = isRecord(item.ListingDetails) ? item.ListingDetails : null
  const startTime = stringValue(listingDetails?.StartTime)
  ...
  const card: Card = {
    ...
    status:
      summary.quantityAvailable > 0 || summary.quantity > 0
        ? "available"
        : "sold",
    images: images.length > 0 ? images : ["https://i.ebayimg.com/images/g/placeholder/s-l1600.jpg"],
    description,
    ebayUrl: `https://www.ebay.com/itm/${itemId}`,
    ebayItemId: itemId,
    sku: summary.sku || stringValue(item.SKU) || undefined,
    dateAdded: getDateAdded(item),
  }
...
export function mergeWithExistingInventory(
  syncedCards: Card[],
  existingCards: Card[]
): Card[] {
  const activeItemIds = new Set(
    syncedCards.map((card) => card.ebayItemId).filter(Boolean)
  )

  const preservedSold = existingCards.filter((card) => {
    if (card.status !== "sold") return false
    if (card.ebayItemId && activeItemIds.has(card.ebayItemId)) return false
    return true
  })
